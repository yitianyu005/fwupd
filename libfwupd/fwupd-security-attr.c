...
/**
 * fwupd_security_attr_get_summary:
 * @self: A #FwupdSecurityAttr
 *
 ...
 **/
const gchar *
fwupd_security_attr_get_summary (FwupdSecurityAttr *self)
{
	FwupdSecurityAttrPrivate *priv = GET_PRIVATE (self);
	g_return_val_if_fail (FWUPD_IS_SECURITY_ATTR (self), NULL);
	return priv->summary;
...
/**
 * fwupd_security_attr_set_summary:
 * @self: A #FwupdSecurityAttr
 * @summary: the attribute one line summary
 *
 ...
 **/
void
fwupd_security_attr_set_summary (FwupdSecurityAttr *self, const gchar *summary)
{
	FwupdSecurityAttrPrivate *priv = GET_PRIVATE (self);
	g_return_if_fail (FWUPD_IS_SECURITY_ATTR (self));
	g_free (priv->summary);
...
/**
 * fwupd_security_attr_get_number:
 * @self: A #FwupdSecurityAttr
 *
 * Gets the HSI number.
 *
 ...
 **/
guint32
fwupd_security_attr_get_number (FwupdSecurityAttr *self)
{
	FwupdSecurityAttrPrivate *priv = GET_PRIVATE (self);
	g_return_val_if_fail (FWUPD_IS_SECURITY_ATTR (self), 0);
	return priv->number;
...
/**
 * fwupd_security_attr_set_number:
 * @self: A #FwupdSecurityAttr
 * @number: HSI number
 *
 * Sets the HSI number. A @number of 0 is not used for the HSI calculation.
 *
 * Since: 1.4.2
 **/
void
fwupd_security_attr_set_number (FwupdSecurityAttr *self, guint32 number)
{
	FwupdSecurityAttrPrivate *priv = GET_PRIVATE (self);
	g_return_if_fail (FWUPD_IS_SECURITY_ATTR (self));
	priv->number = number;
...
fwupd_security_attr_from_key_value (FwupdSecurityAttr *self, const gchar *key, GVariant *value)
{
	...
	if (g_strcmp0 (key, FWUPD_RESULT_KEY_SUMMARY) == 0) {
		fwupd_security_attr_set_summary (self, g_variant_get_string (value, NULL));
		return;
	}
	...
	if (g_strcmp0 (key, FWUPD_RESULT_KEY_HSI_NUMBER) == 0) {
		fwupd_security_attr_set_number (self, g_variant_get_uint32 (value));
		return;
	}
}
...
fwupd_security_attr_to_json (FwupdSecurityAttr *self, JsonBuilder *builder)
{
	...
	fwupd_security_attr_json_add_int (builder, FWUPD_RESULT_KEY_HSI_NUMBER, priv->number);
	fwupd_security_attr_json_add_string (builder, FWUPD_RESULT_KEY_NAME, priv->name);
	fwupd_security_attr_json_add_string (builder, FWUPD_RESULT_KEY_SUMMARY, priv->summary);
